...
export const backendApi = {
  ...
  updateProjectStatus: (name, status) =>
    request(`/api/projects/${encodeURIComponent(name)}`, { method: "PATCH", body: JSON.stringify({ status }) }),
  extractFile: (fileName, mimeType, text, dataUrl, projectOptions = []) =>
    request("/api/file-extract", {
      method: "POST",
      body: JSON.stringify({ fileName, mimeType, text, dataUrl, projectOptions }),
    }),
  createMilestone: (values) => request("/api/milestones", { method: "POST", body: JSON.stringify(values) }),
  updateMilestone: (id, values) =>
    request(`/api/milestones/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(values) }),
  ...
  updateTask: (id, task) =>
    request(`/api/tasks/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(task) }),
  ...
  approveSourceItem: (sourceItemId, extractedIndex, values) =>
    request(`/api/source-items/${encodeURIComponent(sourceItemId)}/approve`, {
      method: "POST",
      body: JSON.stringify({ extractedIndex, values }),
    }),
  dismissSourceItem: (sourceItemId, extractedIndex) =>
    request(`/api/source-items/${encodeURIComponent(sourceItemId)}/dismiss`, {
      method: "POST",
      body: JSON.stringify({ extractedIndex }),
    }),
};
