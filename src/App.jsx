  ...
  return <div className="app-shell">
    ...
    <div className="main-column">
      ...
      <main className="content">
        ...
        {screen === "milestones" && <MilestonesView milestones={milestones} tasks={tasks} projects={activeProjectNames(projects)} onAdd={addMilestone} onUpdate={updateMilestone} />}
    ...
    {modal && <Modal modal={modal} onClose={() => setModal(null)} onApprove={approveDraft} onDismiss={dismissQueueItem} onNotice={showNotice} onNewTask={createTask} onUpdateTask={updateTask} projectOptions={activeProjectNames(projects)} />}
...
function MilestonesView({ milestones, tasks, projects, onAdd, onUpdate }) {
  ...
  const renderItems = (items, emptyTitle, emptyCopy) => view === "timeline" ? <MilestoneTimeline milestones={items} projects={projectOptions} onUpdate={onUpdate} emptyTitle={emptyTitle} emptyCopy={emptyCopy} /> : <MilestoneList milestones={items} projects={projectOptions} sort={sort} onSort={toggleSort} onUpdate={onUpdate} emptyTitle={emptyTitle} emptyCopy={emptyCopy} />;
...
function MilestoneTimeline({ milestones, projects, onUpdate, emptyTitle = "No milestones yet", emptyCopy = "Add one above to start your timeline." }) {
  return <div className="milestone-timeline">{milestones.map((milestone) => <MilestoneTimelineEntry key={milestone.id} milestone={milestone} projects={projects} onUpdate={onUpdate} />)}{!milestones.length && <EmptyState icon={CalendarBlank} title={emptyTitle} copy={emptyCopy} />}</div>;
}

function MilestoneTimelineEntry({ milestone, projects, onUpdate }) {
  return <article className={`milestone-entry urgency-${milestoneUrgency(milestone)}`}><span className="milestone-dot" /><div className="milestone-date">{milestone.date}</div><MilestoneCard milestone={milestone} projects={projects} onUpdate={onUpdate} /></article>;
}

function MilestoneCard({ milestone, projects, onUpdate }) {
...
function WorkRegister({ tasks, projects, projectFilter, setProjectFilter, newTaskRequest, onConsumeNewTaskRequest, onCreateTask, onDone, onClearBlocker, onUpdate, onCreateSubtask, onRefresh, onEdit }) {
  ...
  const visible = useMemo(() => tasks.filter((task) => projectFilter === "All projects" || task.project === projectFilter).sort((left, right) => {
...
function Modal({ modal, onClose, onApprove, onDismiss, onNotice, onNewTask, onUpdateTask, projectOptions }) {
