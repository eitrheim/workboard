...
function App() {
  ...
  const deleteTask = async (task) => {
    ...
    } catch (error) { showNotice(error.message || "Could not remove the task"); }
  };
  ...
  return <div className="app-shell">
    ...
    <div className="main-column">
      ...
      <main className="content">
        {!(["milestones", "project"].includes(screen)) && <div className={`content-heading ${screen === "register" ? "register-heading" : ""}`}><div><p className="eyebrow">{todayLongLabel()}</p><h1>{navItems.find((item) => item.id === screen)?.label}</h1></div><div className="heading-actions">{screen === "register" ? <><button className="secondary-button register-header-action" onClick={refreshRegister} disabled={refreshing}><ArrowClockwise size={17} /> {refreshing ? "Refreshing" : "Refresh"}</button><button className="secondary-button register-header-action" onClick={() => setModal({ type: "recurring-task" })}><Clock size={17} /> New repeat task</button><button className="primary-button new-task-button" onClick={() => setNewTaskRequest((value) => value + 1)}><Plus size={18} weight="bold" /> <span className="desktop-only">New task</span><span className="mobile-only">Add</span></button></> : <button className="primary-button new-task-button" onClick={() => setModal({ type: "new-task" })}><Plus size={18} weight="bold" /> <span className="desktop-only">New task</span><span className="mobile-only">Add</span></button>}</div></div>}
        {screen === "overview" && <Overview queue={queue} tasks={tasks} completed={completed} scoreEvents={scoreEvents} milestones={milestones} onNavigate={setScreen} />}
    ...
    {modal && <Modal modal={modal} onClose={() => setModal(null)} onApprove={approveDraft} onDismiss={dismissQueueItem} onNotice={showNotice} onNewTask={createTask} onCreateRecurring={createRecurringTask} onUpdateTask={updateTask} onDeleteTask={deleteTask} projectOptions={activeProjectNames(projects)} />}
...
function Overview({ queue, tasks, completed, scoreEvents, milestones, onNavigate }) {
  ...
  const upcomingMilestones = [...milestones].filter((milestone) => milestone.dateKey && milestone.dateKey >= todayKey()).sort((left, right) => left.dateKey.localeCompare(right.dateKey)).slice(0, 2);
  const queueDetail = queue.length === 0 ? "items to review; you're caught up" : `${queue.length === 1 ? "item" : "items"} to review`;
  return <section className="screen-content overview-screen"><div className="overview-intro"><p>One place to see what needs your attention and how your week is moving.</p></div><div className="overview-grid"><OverviewCard icon={Archive} label="Review queue" value={`${queue.length}`} detail={queueDetail} tone="queue" onClick={() => onNavigate("review")} /><OverviewCard icon={ListChecks} label="Work register" value={`${dueToday}`} detail={`${dueToday === 1 ? "item" : "items"} due today`} tone="register" onClick={() => onNavigate("register")} /><OverviewMilestoneCard milestones={upcomingMilestones} onClick={() => onNavigate("milestones")} /><button className="overview-card overview-score-card" onClick={() => onNavigate("completed")} type="button"><div className="overview-card-top"><span className="overview-card-label"><CheckCircle size={19} /> Score</span><ArrowRight size={20} /></div><strong>{periodScore} pts</strong><p>Past 7 days</p><ScoreLineChart points={scoreDays} /><div className="score-axis">{scoreDays.map((day) => <span key={day.short}>{day.label}</span>)}</div></button></div></section>;
}
...
function OverviewMilestoneCard({ milestones, onClick }) { return <button className="overview-card overview-milestone-card" onClick={onClick} type="button"><div className="overview-card-top"><span className="overview-card-label"><CalendarBlank size={19} /> Milestones</span><ArrowRight size={20} /></div><div className="overview-milestone-list">{milestones.length ? milestones.map((milestone) => <div className={`overview-milestone-item urgency-${milestoneUrgency(milestone)}`} key={milestone.id}><strong>{milestone.name}</strong><span>{milestoneTimingLabel(milestone)}</span></div>) : <p className="overview-empty-milestones">No upcoming milestones</p>}</div></button>; }
...
function ScoreLineChart({ points }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      context.stroke();
      points.forEach((point, index) => { context.fillStyle = "#fff"; context.beginPath(); context.arc(x(index), y(point.value), 4, 0, Math.PI * 2); context.fill(); context.fillStyle = "#5c2f9e"; context.beginPath(); context.arc(x(index), y(point.value), 2.5, 0, Math.PI * 2); context.fill(); });
    };
    draw();
...
function TaskExtraction({ projects, onSendToQueue, onRemove, onNotice }) {
  ...
  return <section className="screen-content extraction-screen">
    ...
    <div className="extraction-layout">
      ...
      <section className="extraction-pane items-pane" aria-label="Extracted tasks and milestones"><div className="extracted-header"><div><span className="eyebrow">{fileState ? `Extracted from ${fileState.name}` : "Extracted items"}</span><h2>{fileState ? `${items.length} ${items.length === 1 ? "item" : "items"} found` : "Nothing extracted yet"}</h2></div>{fileState && <button className="primary-button extracted-approve-all" disabled={!items.length || extractingFile} onClick={approveAll}><Check size={16} /> Approve all</button>}</div>{fileState && extractingFile && <p className="extraction-method-note processing-note" role="status"><Sparkle size={14} /> AI is still reviewing the document. Please wait; tasks, milestones, dates, owners, and supporting evidence are being extracted.</p>}{!fileState ? <div className="extracted-empty"><ListChecks size={28} /><p>Items will appear here after you drop in a file.</p></div> : <div className="extracted-list">{items.map((item) => <ExtractedItemCard item={item} key={item.id} onApprove={() => approveOne(item)} onEdit={() => approveOne(item)} onRemove={() => removeOne(item)} />)}{!items.length && <div className="extracted-empty compact">{extractingFile ? <><Sparkle size={27} /><p>Waiting for the backend to finish the deep pass...</p></> : <><CheckCircle size={27} /><p>All extracted items are handled.</p></>}</div>}</div>}</section>
    </div>
    {fileState && <div className="extraction-bottom-bar"><div><strong>{extractingFile ? "Processing document..." : items.length ? `${items.length} items ready` : "Extraction complete"}</strong><span>{extractingFile ? "Please wait while AI completes its deep pass." : "Approve sends an item to the Review queue. Edit also sends it there for full editing."}</span></div></div>}
  </section>;
}
...
function ExtractedItemCard({ item, onApprove, onEdit, onRemove }) {
  ...
  return <article className={`extracted-item-card ${missing.length ? "missing-item" : ""}`}><span className="extracted-item-select" aria-hidden="true" /><div className="extracted-item-main"><div className="extracted-item-title-row"><h3>{item.title}</h3><span className={`extraction-confidence ${missing.length ? "needs-details" : "high-confidence"}`}>{confidence}</span></div><div className="extracted-item-meta"><span>{item.project || "Project not found"}</span><span>Owner: {item.owner || "Not found"}</span><span className={!item.dateLabel ? "missing-date-chip" : ""}>{item.dateLabel ? `Due ${item.dateLabel}` : "No date found"}</span></div><div className="extracted-item-actions"><button className="extract-card-approve" onClick={onApprove}>Approve</button><button className="extract-card-edit" onClick={onEdit}>Edit</button><button className="extract-card-discard" onClick={onRemove}>Discard</button></div></div></article>;
}
...
function ProjectView({ project, onProjectChange, projects, tasks, completed, milestones, onDone, onClearBlocker, onUpdate, onEdit, onCreateProject, onUpdateProjectStatus }) {
  ...
  return <section className="screen-content project-screen">
    <div className="project-view-header">
      ...
      <div className="project-stats" aria-label={`${project} project stats`}><div className="project-stat project-open-stat"><span>Open</span><strong><span className="project-stat-value">{projectTasks.length}</span><span className="project-stat-unit">Tasks</span></strong></div><div className="project-stat project-completed-stat"><span>Completed</span><strong><span className="project-stat-value">{projectScore}</span><span className="project-stat-unit">Pts</span></strong></div><div className="project-stat project-next-stat"><span>Next milestone</span>{nextMilestone ? <><strong>{nextMilestone.name}</strong><small>{nextMilestone.date} · {milestoneTimingLabel(nextMilestone)}</small></> : <strong>None scheduled</strong>}</div></div>
    </div>
    <section className="project-milestones-section"><div className="section-title-row"><div><h2>Milestones &amp; deadlines</h2><p className="section-subtitle">Key dates scoped to {project}</p></div></div><div className="project-milestones-strip">{projectMilestones.map((milestone) => <article className={`project-milestone-card urgency-${milestoneUrgency(milestone)}`} key={milestone.id}><span className={`project-milestone-dot urgency-${milestoneUrgency(milestone)}`} /><div><strong>{milestone.name}</strong><span className="project-milestone-meta">{milestone.date} · {milestone.type} · {milestone.linkedCount} linked {milestone.linkedCount === 1 ? "task" : "tasks"}</span><span className={`project-milestone-timing urgency-${milestoneUrgency(milestone)}`}>{milestoneTimingLabel(milestone)}</span></div></article>)}{!projectMilestones.length && <p className="project-empty-copy">No milestones or deadlines are linked to this project yet.</p>}</div></section>
...
function WorkRegister({ tasks, projects, projectFilter, setProjectFilter, newTaskRequest, onConsumeNewTaskRequest, onCreateTask, onDone, onClearBlocker, onUpdate, onCreateSubtask, onEdit }) {
  ...
  const registerSummary = useMemo(() => {
    ...
    return {
      ...
      dueThisWeek: dated.filter((task) => task.deadlineKey >= today && task.deadlineKey <= addDaysToKey(today, 7)).length,
    };
  }, [tasks]);
...
function RegisterRow({ task, projectOptions, onDone, onClearBlocker, onUpdate, onCreateSubtasks, hasDraftSubtasks, onEdit, showSubtaskAction = true }) {
  ...
  return <Fragment><div className={`mobile-register-card ${task.status === "blocked" ? "blocked-row" : ""} ${overdue ? "overdue" : ""}`}><button className="mobile-register-card-content" type="button" onClick={() => onEdit(task)} aria-label={`Open details for ${task.title}`}><div className="mobile-task-head">{dueToday && <Warning size={19} className="red-icon" />}{task.status === "blocked" && <span className="blocked-indicator"><Warning size={18} /></span>}<strong>{draft.title}</strong>{task.recurring && <span className="recurring-label">Weekly</span>}</div><div className="mobile-task-meta"><span>{draft.project || "No project"}</span><span className={dueToday ? "mobile-due-chip due" : !draft.deadlineKey ? "mobile-due-chip missing" : "mobile-due-chip"}><CalendarBlank size={14} />{deadlineLabel}</span><span className={ownerMissing ? "mobile-missing-detail" : ""}>{ownerMissing ? "Unassigned" : draft.owner}</span><span className={effortMissing ? "mobile-missing-detail" : ""}>{effortMissing ? "No effort" : `${draft.effortHours} hr${Number(draft.effortHours) === 1 ? "" : "s"}`}</span></div>{task.blocker && <span className="mobile-blocker-copy">{task.blocker}</span>}</button><div className="mobile-register-actions">{showSubtaskAction && Number(draft.effortHours) > 1 && <button className="subtask-button icon-action-button" title={hasDraftSubtasks ? "Regenerate subtasks" : "Create subtasks"} aria-label={hasDraftSubtasks ? "Regenerate subtasks" : "Create subtasks"} onClick={() => onCreateSubtasks({ ...task, ...draft })}><Sparkle size={16} /></button>}{task.status === "blocked" ? <button className="mobile-clear-blocker" type="button" onClick={() => onClearBlocker(task)}>Clear blocker</button> : <button className="icon-action-button done-action-button" type="button" title="Mark done" aria-label={`Mark ${task.title} done`} onClick={() => onDone(task)}><Check size={18} weight="bold" /></button>}</div></div><div className={`table-row desktop-register-row ${task.status === "blocked" ? "blocked-row" : ""}`}><div className="task-cell register-edit-cell">{dueToday && <Warning size={20} className="red-icon" />}{task.status === "blocked" && <span className="blocked-indicator"><Warning size={19} /></span>}<input className="inline-input task-input" value={draft.title} onChange={(event) => update("title", event.target.value)} aria-label={`Task name for ${task.title}`} />{task.recurring && <span className="recurring-label">Weekly</span>}{task.blocker && <small>{task.blocker}</small>}<button className="row-edit-details" onClick={() => onEdit(task)} aria-label={`Open full details for ${task.title}`}><PencilSimple size={14} /></button></div><label className="register-edit-cell" data-label="Project"><span className="sr-only">Project</span><select className="inline-input project-inline-select" value={draft.project} onChange={(event) => update("project", event.target.value)}>{projectOptions.includes(draft.project) || !draft.project ? <option value="">{draft.project || "Select active project"}</option> : <option value={draft.project}>{draft.project} · Finished</option>}{projectOptions.filter((project) => project !== draft.project).map((project) => <option key={project} value={project}>{project}</option>)}</select></label><label className={`register-edit-cell ${dueToday ? "due-today" : !draft.deadlineKey ? "no-deadline" : ""}`} data-label="Deadline"><span className="sr-only">Deadline</span><input className="inline-input" type="date" value={draft.deadlineKey || ""} onChange={(event) => update("deadline", event.target.value)} /></label><label className={`register-edit-cell ${ownerMissing ? "unassigned-owner" : ""}`} data-label="Owner"><span className="sr-only">Owner</span><input className="inline-input" value={draft.owner} onChange={(event) => update("owner", event.target.value)} /></label><label className={`register-edit-cell ${effortMissing ? "zero-effort" : ""}`} data-label="Effort"><span className="sr-only">Effort</span><input className="inline-input effort-input" type="number" min="0.25" step="0.25" inputMode="decimal" value={draft.effortHours} onChange={(event) => update("effortHours", event.target.value)} /></label><div className="register-row-actions">{showSubtaskAction && Number(draft.effortHours) > 1 && <button className="subtask-button icon-action-button" title={hasDraftSubtasks ? "Regenerate subtasks" : "Create subtasks"} aria-label={hasDraftSubtasks ? "Regenerate subtasks" : "Create subtasks"} onClick={() => onCreateSubtasks({ ...task, ...draft })}><Sparkle size={16} /></button>}{changed && <button className="save-row-button" onClick={save}>Save</button>}{task.status === "blocked" ? <button className="row-action" onClick={() => onClearBlocker(task)}>Clear blocker</button> : <button className="row-action icon-action-button done-action-button" title="Mark done" aria-label="Mark done" onClick={() => onDone(task)}><Check size={18} weight="bold" /></button>}</div></div></Fragment>;
}
...
function CompletedView({ completed, scoreEvents = [], tasks, period, setPeriod, onUndo }) {
  ...
  const toggleSort = (key) => setSort((current) => current.key === key ? { key, direction: current.direction === "asc" ? "desc" : "asc" } : { key, direction: "asc" });
  return <section className="screen-content completed-screen"><div className="period-toolbar"><div><p className="eyebrow">Your private score history</p><h2 className="period-title">{period}</h2></div><label className="select-wrap"><span className="sr-only">Summary period</span><select value={period} onChange={(event) => setPeriod(event.target.value)}><option>Last 7 days</option><option>Last 30 days</option></select><CaretDown size={17} /></label></div><div className="metric-grid"><Metric label="Period score" value={`${total} pts`} icon={Sparkle} /><Metric label="Tasks completed" value={`${completedInPeriod.length} tasks`} icon={CheckCircle} /><Metric label="Carryover into period" value={`${carryover} tasks`} note="Open tasks that were already overdue when this period began." icon={StackSimple} /></div><div className="chart-section"><h2>Score by day</h2><div className="chart-plot"><div className="chart-y-axis" aria-hidden="true">{yTicks.map((tick) => <span key={tick}>{tick}</span>)}</div><div className={`bar-chart bar-chart-${periodDays}`} aria-label={`Score by day for the ${period.toLowerCase()}, scaled from zero to ${max} points`}>{bars.map((bar) => { const height = bar.value ? Math.max(4, (bar.value / max) * 100) : 0; const isActive = activeBar === bar.key; return <div className="bar-column" key={bar.key}><div className="bar-track"><button type="button" className={`bar-hit ${isActive ? "active" : ""}`} style={{ "--bar-height": `${height}%` }} onMouseEnter={() => setActiveBar(bar.key)} onMouseLeave={() => setActiveBar(null)} onFocus={() => setActiveBar(bar.key)} onBlur={() => setActiveBar(null)} onClick={() => setActiveBar((current) => current === bar.key ? null : bar.key)} aria-label={`${bar.label}: ${bar.value} points`} aria-pressed={isActive}>{isActive && <span className="bar-tooltip" role="tooltip">{bar.value} {bar.value === 1 ? "point" : "points"}</span>}<span className="bar" style={{ height: `${height}%` }} /></button></div><span>{bar.label}</span></div>; })}</div></div></div><div className="completed-section"><div className="section-title-row"><h2>Task activity</h2></div><div className="completed-table-wrap"><div className="completed-table completed-activity-table" role="table" aria-label="Task score activity"><div className="completed-table-head" role="row"><SortHeader label="Task" sortKey="title" sort={sort} onSort={toggleSort} /><SortHeader label="Project" sortKey="project" sort={sort} onSort={toggleSort} /><SortHeader label="Activity" sortKey="activity" sort={sort} onSort={toggleSort} /><SortHeader label="Date" sortKey="date" sort={sort} onSort={toggleSort} /><SortHeader label="Score" sortKey="points" sort={sort} onSort={toggleSort} /><span /></div>{sortedActivity.map((item) => <div className="completed-table-row" role="row" key={item.id}><strong>{item.title}</strong><span>{item.project}</span><span className="score-activity-label">{item.activity}</span><span>{item.date}</span><strong>{item.points ? `+${item.points} pts` : "No points"}</strong>{item.canUndo ? <button className="secondary-button undone-button" onClick={() => onUndo(item)}>Mark undone</button> : <span />}</div>)}{!sortedActivity.length && <EmptyState icon={CheckCircle} title="No task activity" copy="Completed work and new tasks will appear here." />}</div></div></div></section>;
}
...
function Metric({ label, value, note, icon: Icon }) { return <div className="metric-card"><div className="metric-label"><Icon size={19} /> {label}</div><strong>{value}</strong>{note && <small className="metric-note">{note}</small>}</div>; }

function Modal({ modal, onClose, onApprove, onDismiss, onNotice, onNewTask, onCreateRecurring, onUpdateTask, onDeleteTask, projectOptions }) {
  ...
  if (modal.type === "task-edit") return <TaskForm title="Edit task details" subtitle="Update the task or add context." item={modal.task} projectOptions={projectOptions} onClose={onClose} onSave={(values) => onUpdateTask(modal.task.id, values)} onDelete={() => onDeleteTask(modal.task)} />;
